...
import EnnrollmentForm from 'sections/centerPage/EnnrollmentForm';
...
import MarketplacenModalScreen from 'components/MarketPlaceSliderScreen2';
...
import NewSeoSection1 from './SeoSetion';
import BlogsSectionSeo from './blogSectionSeo';
import PopupTransition from 'components/@extended/Transitions';
import { Dialog } from '@mui/material';
import { DialogContent } from '@mui/material';
import DynamicContent from '@/components/dangrouslyHTML';
import NewSectionSeven from 'sections/home/NewSectionSevenDynamic';
...
const SearchDayCares = ({ abr, daycares, zipcode, aiData, allblogs, topCities }: SearchProps) => {
    ...
    const [zipCodeValidation1, setZipCodeValidation1] = useState<any>(null);
    ...
    const [marketPlacePopup, setMarketPlacePopup] = useState<any>(false);
    ...
    const isZipValid = /^\d+$/.test(`${zipcod}`);
    const showHeader =
        (zipcod && !isZipValid) || !zipcod;

    const categoryLabel = category
        ? category === 'Childcare'
            ? 'Child Care'
            : category + (abr === 'daycare' ? ' Centers' : '')
        : 'Daycares';

    const nearMeLabel = !pathname.includes('nearby')
        ? (isZipValid ? 'Near Me' : '')
        : 'Near Me';

    const locationLabel = zipCode
        ? `${zipCode}${state ? ', ' + state : ''}`
        : cityName;
    const handleConfirm = () => {
        setFields([
            ...
            if (_tmpParamsData && !/^\d+$/.test(`${abr}`)) {
                ...
                if (zipcod && !/^\d+$/.test(`${zipcod}`)) {
                    ...
                    if (_tempCategories.includes(_tempAbr)) {
                        ...
                        if (typeof window !== 'undefined' && window.location.pathname.includes('nearby')) {
                            //   setCityName(_temp + ', ' + daycares);
                            //   handlesearchByParams('', _temp + ', ' + daycares, titleCase(_tempAbr));
                            //   setCategory(titleCase(_tempAbr))
                            // } else {
    ...
    const _aiContent = aiData && wrapContentInDiv(aiData.content);
    const _aboutCity = aiData && wrapContentInDiv(aiData.about_city);
                ...
                if (response.data.status == 'pass') {
                    ...
                    if (_resData?.center?.length > 0) {
                        ...
                        if (_resData?.geo_location?.city) {
                            if (!_zipCode) {
                                const tempLocation = typeof window !== 'undefined' ? window.location.pathname.split('/') : [];
    ...
    const handlesearch = async () => {
        ...
        const data = new FormData();
        if (zipcode == zipCode || formattedCity == cityName) {
            setZipCodeValidation('please change city name or zipcode');
            setIsLoading(false);
            count += 1;
        }
        else if ((zipCode == null || zipCode === '') && (cityName == null || cityName === '')) {
        ...
        axios({
            ...
            .then(async (response) => {
                if (response.data.status == 'fail') {
                    ...
                    setfilteredData([]);
                    setIsMapLoaded(true);
                    setZipCodeValidation1(response?.data?.message);

                }
                if (response.data.status == 'pass') {
                    // setIsLoading(false);
                    const _resData: any = response?.data?.data;
                    setZipCodeValidation1(null);
            ...
            .catch((error) => {
                ...
                setIsLoading(false);
                setZipCodeValidation(null);
                setZipCodeValidation1('Enter valid Zip Code or City Name');
    ...
    //     return _temp;
    // };
    console.log('mui', zipCodeValidation1, zipCode, cityName)
    ...
    return (
        <Stack>
            ...
            {/* {marketPlacePopup && <>
                {!mdScreen ? <MarketplacenModalScreen sliderRatio={sliderRatio}
                    graphData={graphData}
            ...
            {mdScreen && isDetailsModalOpen && <DetailPageMobileView onClose={handleCloseDetailsModal} id={dayCareslistbyId?.id} isMarketPlace={true} />}

            {mdScreen && marketPlacePopup ? <MarketPlaceMobileComp
                onClose={handleMarketPlacePopup}
                sliderRatio={sliderRatio}
                graphData={graphData}
                setEditMarketPlacePopup={setEditMarketPlacePopup}
                ...
                handlesearch={handlesearch}
                handleAddressSearchInput={handleAddressSearchInput}
                location={zipcod ? !/^\d+$/.test(`${zipcode}`) ? (zipcode?.replace(/\b\w/g, (char: any) => char.toUpperCase()).replace(/\-+/g, ' ')) + ', ' + daycares?.toUpperCase() : zipcod : daycares?.replace(/\b\w/g, (char: any) => char.toUpperCase()) + ', ' + abr?.toUpperCase()}
                currentLocation={currentLocation}
                stateRates={stateRates}
                setRemoveBlur={setRemoveBlur}
            ...
            /> :
                <>
                    ...
                    <Grid container sx={{
                            ...
                            <>
                                ...
                                <IconButton
                                    sx={{
                                        ...
                                        width: '53px',
                                        height: '28px',
                                        top: '36px', // place it just below the Stack
                                        left: '50%',
                                        transform: 'translateX(-50%)',
                                        zIndex: 9,
                                        backgroundColor: '#000', // optional: to match the bar
                                        color: '#fff',
                                        mt: '4px', // optional spacing
                                    }}
                                >
                                    <ArrowDown2 color='#fff' />
                                </IconButton>
                            </>
                        )} */}
                        {/* {isLoading && 
                        ...
                        <Stack sx={{  mt: { xs: 4, md: 0 }, width: '100%', zIndex: isModalOpen1 && smallScreen ? 0 : 2, px: 2  }}>
                            <Grid
                                container
                                // spacing={2}
                                sx={{
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <Stack pl={2} sx={{ display: { md: 'none', sm: 'block' } }}><FilterAltIcon onClick={() => setOpen(!open)} /></Stack>
                                        <Stack>
                                            {radiusvalidation && (
                                ...
                                {/* {fields[0].date_of_birth && !isSimpleSearch && (
                        ...
                        <Grid item xs={12} display={'flex'} direction={'row'} p={2} sx={{ background: '#fff' }}>

                            {marketPlacePopup && !mdScreen && <>
                                <Slide
                                    direction={marketPlacePopup ? 'right' : 'left'}
                                    in={marketPlacePopup}
                                    mountOnEnter
                                    unmountOnExit
                                    timeout={500}
                                    ...
                                    <Grid>
                                        <Stack
                                            sx={{
                                                ...
                                                minWidth: { md: '420px', xs: '100vw' },
                                                zIndex: 1,
                                                height: fields[0].date_of_birth ? windowSize.innerHeight - 140 : windowSize.innerHeight - 100,   //here for mobile screen heigh
                                                maxHeight: { md: fields[0].date_of_birth ? '100%' : '100%', xs: '100%' },
                                                background: '#fff',
                                                marginTop: { md: fields[0].date_of_birth && marketPlacePopup && !isSimpleSearch ? '130px' : '68px', xs: '0px' },
                                                paddingTop: { xs: fields[0].date_of_birth ? '150px' : '140px', md: '0px' },
                                                px: 1,
                                                boxShadow: 4
                                            }}
                                        ><MarketplacenModalScreen sliderRatio={sliderRatio}
                                            graphData={graphData}
                                            setEditMarketPlacePopup={setEditMarketPlacePopup}
                                            editMarketPlacePopup={editMarketPlacePopup}
                                            // setWelcomePopup={setWelcomePopup}
                                        ...
                                        {/* <Stack
                                                sx={{
                                                    ...
                                                    position: 'absolute',
                                                    left: { md: '420px', sm: '0px' },
                                                    top: '50%',
                                                    transform: 'translateY(-50%)', // Center vertically
                                                    width: '8px',
                                                    // height: '200px', // Set height for the white background
                                                    zIndex: 999,
                                                    background: '#fff',
                                                    p: '10px 12px',
                                                    cursor: 'pointer'
                            ...
                            {/* {!marketPlacePopup && !mdScreen && <Stack
                                sx={{
                                    display: 'flex',
                                    justifyContent: 'center',
                                    alignItems: 'center',
                            ...
                            {infoPopup && !mdScreen && marketPlacePopup &&
                            ...
                            {infoPopup && !mdScreen && marketPlacePopup && <>
                                ...
                                <Stack
                                    sx={{
                                        ...
                                        position: 'absolute',
                                        left: '460px',
                                        width: { sm: '512px', xs: '100%' },
                                        minWidth: { md: '512px', xs: '100vw' },
                                        zIndex: 9,
                                        background: '#fff',
                                        marginTop: { md: '10%' },
                                        p: 2,
                                ...
                                >
                                    ...
                                    <Stack sx={{ mr: 2, py: 0 }}>
                                        ...
                                        <Typography sx={{ fontSize: '14px', fontWeight: '400', mt: 1 }}>
                                        ...
                                        <Stack
                                            ...
                                            mt={1}
                                        >
                                            <Typography sx={{ fontSize: '16px', fontWeight: '600' }}> Find the perfect daycare with confidence using our Smart Search</Typography>
                                            <Typography sx={{ fontSize: '14px', fontWeight: '400' }}>

                                                Designed to match parents with trusted childcare providers based on location, age group, hours, programs, safety ratings, and real parent reviews.


                                            </Typography>
                                            <Typography sx={{ fontSize: '16px', fontWeight: '600' }}>  What sets us apart?</Typography>
                                            <Typography sx={{ fontSize: '14px', fontWeight: '400' }}>
                                            ...
                                            <Typography sx={{ fontSize: '14px', fontWeight: '400' }}>  We'll <strong>negotiate</strong> on your behalf to help you secure the best available rates.

                                            </Typography>
                                            <Typography sx={{ fontSize: '14px', fontWeight: '400' }}>Whether you need full-time care, flexible hours, or special programs, Smart Search saves you time, stress, and money by connecting you with quality daycare that fits your needs and budget.</Typography>
                                            {/*   <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', }}>
                                                    <img src={gcheck} alt='greencheckicon' style={{ height: '13px', width: '13px' }} />
                                                    <Typography sx={{ fontSize: '14px', fontWeight: { md: 500, xs: 400 }, ml: 1 }}>Our <span style={{ fontWeight: 600 }}>CHILDCARE SEARCH MARKETPLACE</span></Typography>
                                                </Stack>

                                                <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', mt: 1 }}>
                                                    <img src={gcheck} alt='greencheckicon' style={{ height: '13px', width: '13px' }} />
                                                    <Typography sx={{ fontSize: '14px', fontWeight: { md: 500, xs: 400 }, ml: 1 }}>Connects families with a  <span style={{ fontWeight: 600 }}>NETWORK OF PROVIDERS</span></Typography>
                                                </Stack>
                                                <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', mt: 1 }}>
                                                    <img src={gcheck} alt='greencheckicon' style={{ height: '13px', width: '13px' }} />
                                                    <Typography sx={{ fontSize: '14px', fontWeight: { md: 500, xs: 400 }, ml: 1 }}>Ensuring access to  <span style={{ fontWeight: 600 }}>QUALITY, AFFORDABLE CARE</span></Typography>
                                                </Stack>
                                                <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', mt: 1 }}>
                                                    <img src={gcheck} alt='greencheckicon' style={{ height: '13px', width: '13px' }} />
                                                    <Typography sx={{ fontSize: '14px', fontWeight: { md: 500, xs: 400 }, ml: 1 }}>Parents can  <span style={{ fontWeight: 600 }}>SEARCH, COMPARE, SELECT AND NEGOTIATE</span></Typography>
                                                </Stack>
                                                <Stack pl={2}><Typography sx={{ fontSize: '14px', fontWeight: '400' }}>(from a wide range of Trusted childcare options)</Typography></Stack> */}
                                        </Stack>
                                    ...
                                    <Stack display={'none'}>
                                        <Typography variant='p1'>What sets us apart? We negotiate on your behalf. By leveraging our marketplace model, we create a competitive environment where providers offer their best rates to attract families. This means parents don't have to spend time negotiating-our platform negotiating-our platform ensures  transparent pricing and helps secure cost-effective childcare solutions without compromising quality. With our service, finding safe, reliable, and budget-friendly childcare has never been easier! </Typography>
                                    </Stack></Stack>
                            </>}
                            ...
                            <Stack
                                sx={{
                                    ...
                                    // position: 'absolute',
                                    // left: { md: marketPlacePopup ? '440px' : '12px', sm: '0px' },
                                    width: '50vw',
                                    minWidth: '30vw',
                                    zIndex: 1,
                                    height: height,
                                    background: '#fff',
                                    // background: !fields[0].date_of_birth && marketPlacePopup && isSimpleSearch ? '#ccc' : '#fff',
                                    // marginTop: { md: fields[0].date_of_birth && marketPlacePopup && !isSimpleSearch ? '130px' : '68px', xs: '0px' },
                                    // paddingTop: { xs: fields[0].date_of_birth ? '150px' : '140px', md: '0px' },
                                    px: 1,
                                    filter: removeBlur ? 'blur(10px)' : 0,
                                    overflow: 'hidden'
                                    // opacity: !fields[0].date_of_birth && marketPlacePopup && isSimpleSearch ? '0.6' : 1,
                                }}
                            >
                                {(zipCode && zipCode != 'No results') || (cityName && cityName != 'No results') ? (
                                    <Stack
                                        sx={{
                                            ...
                                            padding: '5px 10px',
                                            width: '100%',
                                            paddingTop: { xs: fields[0].date_of_birth ? '45px' : '5px', md: '18px' }
                                        }}
                                    >
                                        {!zipCodeValidation1 && <Typography mt={0} variant="bodytext1" sx={{ maxWidth: '270px' }}>
                                            {`${categoryLabel} ${nearMeLabel} in ${locationLabel}`}
                                        </Typography>}
                                        {filteredData?.length == 0 && (
                                            <div style={{ justifyContent: 'space-between', alignItems: 'center', padding: '5px 10px' }}>
                                                {zipCodeValidation1 ? <Typography mt={2} sx={{ fontSize: '14px', fontWeight: 'bold', color: 'red' }}>
                                                    {zipCodeValidation1}
                                                </Typography> : <Typography mt={2} sx={{ fontSize: '12px', fontWeight: 'bold' }}>
                                    ...
                                    >
                                        <Stack sx={{ mb: 2 }}>
                                            {Array.isArray(filteredData) && filteredData.length != 0 &&
                                                zipCode && zipcod && /^\d+$/.test(`${zipcod}`) ? <>
                                                <Typography variant="bodytext1" sx={{ px: '10px' }} component="h1">
                                                    {filteredData?.filter(item => item.zip_code == zipcod).length} Daycares in  {zipcod}
                                                </Typography>
                                                {filteredData?.filter(item => item.zip_code == zipcod).map((item: any, ind: any) => (
                                                    <Stack key={ind}>
                                                        <Box
                                                        ...
                                                        >
                                                            ...
                                                            <Stack sx={{ display: 'flex', flexDirection: 'row', m: 2 }}>
                                                                <a
                                                                    href={!removeBlur ? googleMapsUrl : '#'}
                                                                    target={!removeBlur ? "_blank" : ''}
                                                         ...
                                                         <Typography variant="bodytext" sx={{ pl: 1 }}>
                                                                          The Learning Experience- Green Valley Ranch provides quality early education in Denver, 80249. Explore information such as Address, Operating Hours, Contact info, Licensing, Age Serving, Rating, Financial Aid and much more! Program provides opportunities for recreation, enrichment, and character building with flexible care schedules. Use the Pre-Enrollment form to enroll your child in this care facility.
                                                                        </Typography>
                                                        <Stack
                                                            sx={{
                                                ...
                                                {filteredData?.filter(item => item.zip_code != zipcod).length != 0 && <> <Typography variant="bodytext1" component="h1" my={2}>
                                                    {filteredData?.filter(item => item.zip_code != zipcod).length} Daycares Near {zipcod} ({selectedMiles ? selectedMiles : 10}mi Radius)
                                                </Typography>
                                                    {filteredData?.filter(item => item.zip_code != zipcod).map((item: any, ind: any) => (
                                                        <Stack key={ind}>
                                                            <Box
                                                ...
                                                :
                                                filteredData && filteredData.length != 0 && filteredData?.map((item: any, ind: any) => (
                                                    <Stack key={ind}>
                                                        <Box
                            ...
                            <Stack sx={{
                                ...
                                <div>
                                    ...
                                    {isLoading ? <Stack
                                        sx={{
                                            height: '110vh',  //here for mobile screen heigh
                                            width: '100%',
                                            backgroundColor: '#ccc',
                                            opacity: '0.8',
 borderRadius: '0px 20px 20px 0px',
                                boxShadow: '0px 4px 8px rgba(2, 2, 2, 0.1)',
                                            justifyContent: 'center',
                                            alignItems: 'center'
                                        }}
                                    >
                                        <CircularProgress size={50} style={{ color: '#fff', display: 'none' }} />
                                    </Stack> :
                                        ...
                                        <GoogleMap
                                            mapContainerStyle={{
                                                height: '113vh',  //here for mobile screen heigh
                                                width: !open ? '100%' : 'auto',
 borderRadius: '0px 20px 20px 0px',
                                boxShadow: '0px 4px 8px rgba(2, 2, 2, 0.1)',
                                                // filter: (!mdScreen && removeBlur) ? "blur(8px)" : '0px'
                                                // opacity: isLoading || !fields[0].date_of_birth && marketPlacePopup && isSimpleSearch ? '0.5' : '1'
                                            }}
                                            options={{
                                                styles: mapStyles,
                        ...
                        {/* </Grid> */}
                        {/* Desktop Modal box */}
                        {!isLoading && ShowCenterDetails && !removeBlur && (
                            <Slide
                                direction={'right'}
            ...
            >
                ...
                {!/^\d+$/.test(`${zipcod}`) && <Container sx={{ my: 5 }}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={aiAboutCity ? 12 : 12}>


                            <Stack>
                                <Typography variant="sectiontitle" sx={{ fontSize: '20px', mb: 2 }}>Useful Information</Typography>
                                {/* <MainCard title="UseFul Information" variant="sectiontitle"> */}
                                <Stack >

                                    {aiContent ?
                                        _aiContent && <Typography
                                            sx={{ fontSize: '14px' }}
                                        ...
                                        />
                                        : _aiContent && <DynamicContent htmlContent={_aiContent} />
                                    }
                        ...
                        {_aboutCity && <Grid item xs={12} md={12}>

                            <Stack >
                                <Typography variant="sectiontitle" sx={{ fontSize: '20px', mb: 2 }}>About City</Typography>
                                ...
                                <Stack >
                                    ...
                                    {/* {_aboutCity &&
                                        ...
                                        />} */}
                                    {_aboutCity && <DynamicContent htmlContent={_aboutCity} />}
                                </Stack>
                                {/* </MainCard> */}
                            </Stack>
                ...
                {zipcod && !/^\d+$/.test(`${zipcod}`) && topCities.length !== 0 && <NewSeoSection1 topCities={topCities} category={category} statecode={daycares} cityname={zipcode} />}
                <BlogsSectionSeo abr={category} allblogs={allblogs} />


                {zipcod && !/^\d+$/.test(`${zipcod}`) ?
                    <>
                        {abr.startsWith('daycare') &&
                            <>
                        ...
                        {abr.startsWith('preschools') &&
                            <>
                                <NewHomesec7 />
                                <SlickSlider />
                            </>
                        }
                        {abr.startsWith('infant') &&
                            <>
                                {matches ? <ParentDashboard /> : <ParentDashboardForHomeMobile />}
                                <SlickSlider />
                            </>
                ...
                {(category || !zipcod) && <NewSectionSeven cityName={cityName} category={category && category?.toLowerCase()} />}
                            ...
                            <Grid container alignItems={'center'} sx={{ alignItems: { xs: 'flex-start', md: 'center' } }}>
                                ...
                                <Grid item xs={10} md={11}>
                                    <Typography variant="bodytext1" sx={{ fontSize: '16px', mb: { xs: 2, md: 0 } }}>
                                        Congratulation on submitting your {marketPlacePopup ? 'Offer' : 'request'}!
                                    </Typography>
                                    <Typography variant="subtitle2" sx={{ fontSize: '14px' }}>
                                        We are going to negotiate this {marketPlacePopup ? 'Offer' : 'request'} on your behalf with the selected Providers.
                                    </Typography>
                                </Grid>
                            </Grid>
                        </DialogContent>
                ...
                <VideoPopup open={open1} handleClose={handleClose1} url={'childrenkare.mp4'} />
