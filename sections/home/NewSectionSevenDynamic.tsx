...
import { getFaqsByCategory } from 'api/faqs';

 function NewSectionSeven({ cityName, category }: { cityName?: string; category?: string }) {
//   const data = [
//     {
    ...
    <>
      ...
      >
        <Grid item xs={12} sm={12} md={12} lg={7} sx={{ px: { lg: 8, xs: 0 }, py: '100px' }}>
          ...
          <Container sx={{ px: 0 }}>
            {getFaqsByCategory(category, cityName).length != 0 && getFaqsByCategory(category, cityName).map((item, index) => (
