...
const DayCareDetail = ({ centerData }: any) => {
    ...
    const encodedAddress = encodeURIComponent(centerData?.address);
    ...
    return (
        <Grid container sx={{ mt: 7, overflowX: 'hidden', background: '#fff' }}>
            <Grid item xs={12} sm={12} md={12} lg={8} sx={{ pl: { lg: 12, md: 4, xs: 2 }, px: 2 }}>
                <DayCareSecone detailData={centerData} />
                <DayCareSecTwo detailData={centerData} />
                <DayCareSecThree detailData={centerData} />
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={4} sx={{ px: 5, py: 5 }}>
                {!isMarketplace && (
                    <Card>
                        ...
                        <EnnrollmentForm
                            ...
                            setFields={setFields}
                            programs=""
                            questions={(centerData as any)?.ques}
                            centerid={(centerData as any)?.id}
                            dayCareslistbyId={centerData as any}
                        />
                    </Card>
                )}
                <Stack sx={{ py: '4px', mb: 1 }} display={'flex'} direction={'column'} justifyContent={'space-between'} my={1}>
                    ...
                    <Stack sx={{ display: { md: 'block', sm: 'none' } }}>
                        {isLoaded && centerData && Object.keys(centerData).length > 0 && (
                            <Map longi={parseFloat(centerData?.lng)} lati={parseFloat(centerData?.lat)} height={'130px'} />
                        )}
                    </Stack>
                    <Box justifyContent={'space-between'} alignItems={'center'} sx={{ display: { xs: 'block', sm: 'flex' }, mt: 2 }}>
                        <span style={{ display: 'flex', alignItems: 'center', paddingRight: 2 }}>
                            ...
                            >
                                {centerData?.address ? centerData?.address : 'N/A'} <br />
                                {centerData?.city ? centerData?.city : 'N/A'},{centerData?.state ? centerData?.state : 'N/A'}, <br />
                                {centerData?.zip_code ? centerData?.zip_code : 'N/A'}
                            </Typography>
                        </span>
